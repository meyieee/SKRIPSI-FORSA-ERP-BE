...
const REQUEST_TYPE = 'workforce-request';
...
function transformToBackend(frontendData) {
  return transformWorkforceRequestToBackend({
    ...frontendData,
...
function transformToFrontend(backendData) {
  return transformWorkforceRequestToFrontend(backendData);
}

/**
 * Generate reference request number untuk workforce request
...
async function getById(id) {
  ...
  // Validate it's a workforce request
...
/**
 * Get Workforce Request by ref_request_no
...
async function update(frontendData, id) {
  ...
  if (!existing || existing.request_type !== REQUEST_TYPE) {
    throw new Error('Workforce request not found');
  }
